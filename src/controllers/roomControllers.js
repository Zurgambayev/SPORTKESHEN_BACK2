...
export const removeParticipant = async (req, res) => {
    ...
    try {
        ...
        const existingParticipant = await RoomParticipant.findOne({roomId: room._id, userId: req.user._id})
...
export const joinTournament = async (req, res) => {
    ...
    try {
        ...
        const existingParticipant = await RoomParticipant({userId: req.user._id});
        if (existingParticipant) {
            return res.status(409).json({message: "User already in tournament"})
        }
